...
视图方向属于工程语义，不允许由 Agent 用本地坐标自行猜测。

规则：

- 以用户确认的 CAD 界面标准视图为准，例如 Onshape 右上角视图方块的 `左`、`右`、`前`、`后`；
- 若组件方向已经摆正，需要左视图时应在 Onshape 中直接点击 `左`，而不是用本地 `+X/-X` 投影命名；
- 若需要隐藏壳体后讨论内部件，应优先在 CAD 中隐藏明确命名的零件，再截图或导出；
- 在没有可靠零件命名或用户确认前，只能输出候选图，不能把候选图命名为左视图/右视图；
- 不使用 `Onshape part 顺序 + 本地 STEP 实体顺序 + 颜色匹配` 来判断零件身份或左右视图；
- 不使用脱离真实 CAD 视图的概念摆放图作为空间关系决策依据。

原因：

- STEP/Onshape 导入后的实体顺序可能变化；
- 颜色只能辅助观察，不能证明零件身份；
- 装配 transform、flatten、重新导出都会引入识别风险；
- 结构决策依赖真实空间关系，错误视图会直接误导判断。
